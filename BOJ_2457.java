...
public class BOJ_2457 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < m; i++) {
            ...
            // 최소 값 세그먼트 트리에서 주어진 범위안에서의 최소 값 얻기
            int minValue = getValue(minSegmentTree, 1, 0, n - 1, left, right, "min");
            // 최대 값 세그먼트 트리에서 주어진 범위안에서의 최대 값 얻기
    ...
    // 세그먼트 트리를 통해 주어진 범위 안에서 최소, 최대 값 가져오기
    public static int getValue(int[] segmentTree, int node, int start, int end, int left, int right, String minOrMax) {
        // 최소, 최대 값을 탐색할 범위가 노드가 가지는 범위를 벗어난 경우
        if (end < left || right < start) {
            if (minOrMax.equals("min")) {
                // 최소 값 세그먼트 트리를 탐색 중이라면 int타입의 가장 큰 수를 리턴
                return Integer.MAX_VALUE;
            } else {
                // 최대 값 세그먼트 트리를 탐색 중이라면 int타입의 가장 작은 수를 리턴
                return Integer.MIN_VALUE;
            }
        } else if (left <= start && end <= right) {
            // 최소, 최대 값을 탐색할 범위가 노드가 가지는 범위보다 큰 경우 노드 값 리턴
            return segmentTree[node];
        } else {
            // 그 외의 경우는 2가지가 더 존재
            // 최소, 최대 값을 탐색할 범위보다 노드가 가지는 범위가 큰 경우
            // 최소, 최대 값을 탐색할 범위에 노드가 가지는 범위가 일부 포함 되는 경우
            ...
            // 현재 탐색 중인 세그먼트 트리가 최소인지 최대인지 구분
            if (minOrMax.equals("min")) {
                // 최소 값 세그먼트 트리를 탐색 중이라면 최소 값 리턴
                return Math.min(leftNodeValue, rightNodeValue);
            } else {
                // 최대 값 세그먼트 트리를 탐색 중이라면 최대 값 리턴
